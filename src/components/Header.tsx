  ...
  return (
    ...
    >
      <div className="mx-auto flex h-16 max-w-7xl items-center justify-between gap-2 overflow-hidden pl-2 pr-2 sm:h-20 sm:pl-3 sm:pr-3 lg:pl-4 lg:pr-4">
        ...
        {/* Desktop Nav – xl pe hi dikhe taaki overlap na ho */}
        <nav className="hidden items-center justify-center gap-1 xl:flex xl:gap-2">
          {NAV_LINKS.map((item) => (
            <Link
              ...
              className={`relative shrink-0 whitespace-nowrap px-2.5 py-2 text-sm font-medium tracking-wide border-b-2 transition-colors duration-200 xl:px-3 ${
                activeLink === item.label
                  ? "text-red-300 border-red-400"
                  : "text-white/80 border-transparent hover:text-white hover:border-red-400"
              }`}
            >
              {item.label}
              {item.label === "Services" && (
        ...
        {/* Contact – right side, overlap na ho */}
        ...
        <button
          ...
          className="flex h-10 w-10 shrink-0 items-center justify-center rounded-lg bg-white/10 text-white xl:hidden"
      ...
      {/* Mobile menu */}
      {mobileMenuOpen && (
        <div className="border-t border-blue-900/50 bg-[#172554] px-4 py-4 lg:hidden">
            ...
            {NAV_LINKS.map((item) => (
              <Link
                ...
                className={`rounded-lg px-3 py-2.5 text-sm font-medium ${
                  activeLink === item.label ? "bg-white/15 text-red-300" : "text-white"
                }`}
              >
                {item.label}
              </Link>
          ...
          <div className="mt-4 flex flex-col gap-2 border-t border-white/10 pt-4">
            ...
            >
              <span className="text-red-400">📞</span> 011-41001188
            </a>
            <a
              href="mailto:info@carems.in"
              className="flex items-center gap-2 text-sm text-white"
            >
              <span className="text-red-400">✉️</span> info@carems.in
            </a>
          </div>
        </div>
      )}
